...
// Toggle theme (guard if element missing)
if (themeToggle) {
...
window.addEventListener('scroll', () => {
    ...
    navItems.forEach(item => {
        ...
        if (current && href.substring(1) === current) {
            item.classList.add('active');
        }
    });

    // Hide header when scrolled down, show when near top
...
// Animated Counter (runs only once)
...
const animateCounters = () => {
    ...
    const run = () => {
        ...
        if (anyRunning) setTimeout(run, 100);
    };
    run();
};
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (errorMessage) {
                        ...
                        errorMessage.innerHTML = '<i class="fas fa-exclamation-circle"></i> ' + (data.message || 'There was an error sending your message. Please try again.');
                    }
                    if (successMessage) successMessage.style.display = 'none';
                }
            })
...
window.addEventListener('resize', () => {
    ...
    resizeTimeout = setTimeout(createSkillsOrbits, 300);
});
